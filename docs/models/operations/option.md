...
```go
rubago.WithServerURL("https://api.example.com")
```
...
```go
rubago.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
rubago.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
rubago.WithTimeout(30 * time.Second)
```
